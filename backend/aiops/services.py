...
from sxdevops.features import filter_feature_tools
...
def _alert_metric_promql(alert):
    return metric_evidence.alert_metric_promql(alert)


ALERT_METRIC_QUERY_BUDGET = metric_evidence.ALERT_METRIC_QUERY_BUDGET
ALERT_METRIC_SERIES_LIMIT = metric_evidence.ALERT_METRIC_SERIES_LIMIT
ALERT_METRIC_MAX_DURATION_MINUTES = metric_evidence.ALERT_METRIC_MAX_DURATION_MINUTES
ALERT_METRIC_DEFAULT_DURATION_MINUTES = metric_evidence.ALERT_METRIC_DEFAULT_DURATION_MINUTES
ALERT_METRIC_DEFAULT_STEP_SECONDS = metric_evidence.ALERT_METRIC_DEFAULT_STEP_SECONDS


def _safe_float(value, default=None):
    return metric_evidence.safe_float(value, default)


def _promql_escape_label_value(value):
    return metric_evidence.promql_escape_label_value(value)


def _promql_selector(label_values, allowed_labels=None, max_labels=6):
    return metric_evidence.promql_selector(label_values, allowed_labels=allowed_labels, max_labels=max_labels)


def _promql_regex_selector(label_values, allowed_labels=None, max_labels=4):
    return metric_evidence.promql_regex_selector(label_values, allowed_labels=allowed_labels, max_labels=max_labels)


def _promql_with_extra_matchers(selector, extra_matchers):
    return metric_evidence.promql_with_extra_matchers(selector, extra_matchers)


def _alert_metric_label_context(alert):
    return metric_evidence.alert_metric_label_context(alert)


def _metric_plan_item(name, promql, category, intent, weight='medium'):
    return metric_evidence.metric_plan_item(name, promql, category, intent, weight=weight)


def _dedupe_metric_plan(plan, budget=ALERT_METRIC_QUERY_BUDGET):
    return metric_evidence.dedupe_metric_plan(plan, budget=budget)


def _build_alert_metric_query_plan(alert, budget=ALERT_METRIC_QUERY_BUDGET):
    return metric_evidence.build_alert_metric_query_plan(alert, budget=budget)


def _metric_value_from_sample(sample):
    return metric_evidence.metric_value_from_sample(sample)


def _series_numeric_values(series):
    return metric_evidence.series_numeric_values(series)


def _summarize_metric_series(series):
    return metric_evidence.summarize_metric_series(series)


def _metric_label_text(metric):
    return metric_evidence.metric_label_text(metric)


def _summarize_metric_query_result(plan_item, payload, series_limit=ALERT_METRIC_SERIES_LIMIT):
    return metric_evidence.summarize_metric_query_result(plan_item, payload, series_limit=series_limit)


def _format_metric_evidence_item(item):
    return metric_evidence.format_metric_evidence_item(item)


def _alert_metric_time_window(alert, duration_minutes):
    return metric_evidence.alert_metric_time_window(alert, duration_minutes)
...
LIGHTWEIGHT_CHAT_CANNED_MODE = 'canned'
LIGHTWEIGHT_CHAT_MODEL_MODE = 'model'

LIGHTWEIGHT_CHAT_GREETING_TEXTS = {
    '在吗', '在不在', '你好', '您好', 'hi', 'hello', 'hey',
    '哈喽', '嗨', '早', '早上好', '下午好', '晚上好',
}
LIGHTWEIGHT_CHAT_CAPABILITY_TEXTS = {
    '你是谁', '你能做什么', '能做什么', '你会什么', '怎么用',
    '介绍下自己', '介绍一下自己', '有什么功能', '有哪些功能',
}
LIGHTWEIGHT_CHAT_ACK_TEXTS = {
    '谢谢', '谢了', '好的', '好', '嗯', '明白', '知道了', '收到', '可以', '行',
    'ok', 'okay', 'thanks', 'thankyou',
}
LIGHTWEIGHT_CHAT_MODEL_HINT_PATTERN = re.compile(
    r'(你叫什么|你是.*助手|你是.*系统|你是.*平台|你是什么.*系统|你是什么.*平台|'
    r'你.*模型|你.*擅长|怎么称呼|聊聊天|讲个笑话|'
    r'开个玩笑|你还在|还在吗|测试一下|测试你|试试你|^test$|ping$|pong$)',
    re.I,
)
LIGHTWEIGHT_CHAT_OPERATIONAL_DOMAIN_PATTERN = re.compile(
    r'(告警|报警|异常|故障|错误|日志|链路|调用链|'
    r'指标|看板|监控|事件|工单|任务|资源|资产|主机|服务器|机器|节点|'
    r'集群|环境|线上|生产|开发|测试|预发|灰度|'
    r'系统|应用|命名空间|容器|内存|磁盘|网络|端口|进程|中间件|数据库|吞吐|可用性)',
    re.I,
)
LIGHTWEIGHT_CHAT_OPERATIONAL_DOMAIN_WORD_PATTERN = re.compile(
    r'\b(alert|alarm|error|failure|failed|log|trace|tracing|metric|promql|grafana|monitor|'
    r'node|cluster|production|prod|dev|test|staging|uat|namespace|pod|deployment|deploy|service|svc|'
    r'statefulset|daemonset|k8s|kubernetes|kubectl|helm|hbase|regionserver|master|'
    r'zookeeper|zk|docker|container|cpu|memory|disk|network|port|process|mysql|redis|'
    r'nginx|kafka|elasticsearch|postgres|mongodb|qps|sla|slo|agent|skill|mcp|action|llm|'
    r'provider|script|command)\b',
    re.I,
)
LIGHTWEIGHT_CHAT_RISKY_ACTION_PATTERN = re.compile(
    r'(执行|运行|重启|扩容|缩容|发布|部署|安装|修复|回滚|变更|确认|审批|授权|删除|下线|停用)',
    re.I,
)
LIGHTWEIGHT_CHAT_RISKY_ACTION_WORD_PATTERN = re.compile(
...
def _lightweight_chat_mode(question):
    raw_text = str(question or '').strip()
    text = _normalize_light_chat_text(raw_text)
    if not text:
        return ''
    if text in LIGHTWEIGHT_CHAT_GREETING_TEXTS or text in LIGHTWEIGHT_CHAT_CAPABILITY_TEXTS:
        return LIGHTWEIGHT_CHAT_CANNED_MODE
    if text in LIGHTWEIGHT_CHAT_ACK_TEXTS:
        return LIGHTWEIGHT_CHAT_CANNED_MODE
    if LIGHTWEIGHT_CHAT_MODEL_HINT_PATTERN.search(raw_text) or LIGHTWEIGHT_CHAT_MODEL_HINT_PATTERN.search(text):
        return LIGHTWEIGHT_CHAT_MODEL_MODE
    has_operational_domain = (
        LIGHTWEIGHT_CHAT_OPERATIONAL_DOMAIN_PATTERN.search(raw_text)
        or LIGHTWEIGHT_CHAT_OPERATIONAL_DOMAIN_PATTERN.search(text)
        or LIGHTWEIGHT_CHAT_OPERATIONAL_DOMAIN_WORD_PATTERN.search(raw_text)
        or LIGHTWEIGHT_CHAT_OPERATIONAL_DOMAIN_WORD_PATTERN.search(text)
    )
    if has_operational_domain:
        return ''
    has_risky_action = (
        LIGHTWEIGHT_CHAT_RISKY_ACTION_PATTERN.search(raw_text)
        or LIGHTWEIGHT_CHAT_RISKY_ACTION_PATTERN.search(text)
        or LIGHTWEIGHT_CHAT_RISKY_ACTION_WORD_PATTERN.search(raw_text)
...
def _is_lightweight_chat_question(question):
    return bool(_lightweight_chat_mode(question))


def _is_canned_lightweight_chat_question(question):
    return _lightweight_chat_mode(question) == LIGHTWEIGHT_CHAT_CANNED_MODE


def _build_lightweight_chat_result(question):
    text = _normalize_light_chat_text(question)
    if text in LIGHTWEIGHT_CHAT_CAPABILITY_TEXTS:
        content = (
            '我在。你可以直接告诉我环境、系统或资源范围，我可以帮你查询资源、分析告警、关联事件、生成待确认任务草稿。'
        )
    elif text in LIGHTWEIGHT_CHAT_ACK_TEXTS:
        content = '收到。需要继续查询、分析或生成任务时，直接告诉我范围和目标。'
    else:
...
def _build_lightweight_model_chat_result(session, user_message, user, question, provider, runtime_config):
    if not _provider_is_ready(provider):
        result = _build_lightweight_chat_result(question)
        result['metadata']['execution_mode'] = 'lightweight_chat_fallback'
        result['metadata']['processing_steps'][0]['detail'] = '识别为普通聊天，但模型未配置完成，已使用本地轻量回复。'
        return result

    history_messages = _build_history_messages(session, runtime_config, before_message=user_message)
    messages = [
        {
            'role': 'system',
            'content': (
                '你是 damingOps 智能运维平台的智能助手。当前用户输入已被识别为普通聊天或能力咨询，'
                '不是平台取证、资源查询、告警分析、日志分析或执行请求。请用中文自然、简洁回复。'
                '不要声称已经查询平台数据，不要编造集群、节点、告警、日志、任务或成本事实。'
                '如果用户想处理运维问题，引导他说明环境、对象和目标。'
            ),
        },
        *history_messages,
    ...
    completion = _request_model_completion(
        ...
        {
            'model': provider.default_model,
            'temperature': min(float(getattr(provider, 'temperature', 0.2) or 0.2), 0.4),
            'max_tokens': min(int(getattr(provider, 'max_tokens', 800) or 800), 800),
            'messages': messages,
        },
        session=session,
        message=user_message,
    ...
    choice = ((completion or {}).get('choices') or [{}])[0]
    content = _sanitize_assistant_content(_extract_message_content(choice.get('message') or {}))
    if not content:
        content = _build_lightweight_chat_result(question)['content']
    return {
        'content': content,
        'citations': [],
        'tool_calls': [],
        ...
        'pending_action_draft': None,
        'metadata': {
            'execution_mode': 'lightweight_model_chat',
            'processing_steps': [{
                'title': '轻量模型问答',
                'detail': '识别为普通聊天，仅调用模型生成回复，未加载 MCP、Skill、Action 和取证工具链。',
                'status': PROCESSING_STATUS_COMPLETED,
                'timestamp': timezone.now().isoformat(),
            }],
            'tool_events': [],
...
def _build_chat_result(session, user_message, user, question, progress_callback=None, analysis_only=False):
    emit = progress_callback or (lambda **kwargs: None)
    light_chat_mode = _lightweight_chat_mode(question)
    if light_chat_mode == LIGHTWEIGHT_CHAT_CANNED_MODE:
        emit(
            status_value=PROCESSING_STATUS_RUNNING,
            text='已识别为轻量问答',
        )
        return _build_lightweight_chat_result(question)
    if light_chat_mode == LIGHTWEIGHT_CHAT_MODEL_MODE:
        emit(
            status_value=PROCESSING_STATUS_RUNNING,
            text='已识别为普通聊天，正在生成轻量回复',
        )
        try:
            config = get_agent_config()
            ...
            runtime_config = runtime_config_for_agent(config, agent)
            provider = get_active_provider(runtime_config)
            return _build_lightweight_model_chat_result(
                session,
                user_message,
                user,
                question,
            ...
            emit(
                step={'title': '轻量模型问答失败', 'detail': str(exc)[:120], 'status': PROCESSING_STATUS_FAILED},
                text='轻量模型问答已回退',
            )
            result = _build_lightweight_chat_result(question)
            result['metadata']['execution_mode'] = 'lightweight_chat_fallback'
            result['metadata']['error_detail'] = _format_model_call_error(str(exc))
            result['metadata']['processing_steps'][0]['detail'] = '普通聊天模型调用失败，已使用本地轻量回复。'
            return result
        except Exception as exc:
            emit(
                step={'title': '轻量问答异常', 'detail': str(exc)[:120], 'status': PROCESSING_STATUS_FAILED},
                text='问答失败',
            )
