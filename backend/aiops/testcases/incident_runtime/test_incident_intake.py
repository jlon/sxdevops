...
class IncidentAlertIntakeTests(TestCase):
    ...
    def test_webhook_creates_incident_for_prometheus_alert(self):
        ...
        rca_report = task.orchestration_state['rca_report']
        self.assertEqual(rca_report['version'], '1.0')
        self.assertEqual(rca_report['incident']['id'], incident.id)
        self.assertEqual(rca_report['root_cause']['type'], AIOpsIncidentHypothesis.TYPE_ALERT_SYMPTOM)
        self.assertFalse(rca_report['partial'])
        self.assertTrue(rca_report['causal_chain'])
        self.assertTrue(rca_report['evidence']['observed'])
        self.assertTrue(all('payload' not in item for item in rca_report['evidence']['observed']))
        self.assertEqual(task.result_payload['rca_report_version'], '1.0')
        self.assertEqual(task.result_payload['rca_report']['incident_id'], incident.id)
        ...
        self.assertEqual(incident.metadata['last_rca_report']['root_cause']['type'], AIOpsIncidentHypothesis.TYPE_ALERT_SYMPTOM)
        self.assertEqual(incident.metadata['last_investigation']['rca_report']['incident_id'], incident.id)
        ...
        user = User.objects.create_user(username='incident-reader', password='Passw0rd!123')
        user.rbac_roles.add(Role.objects.get(code='platform-admin'))
        token = Token.objects.create(user=user)
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {token.key}')
        detail_response = self.client.get(f'/api/aiops/incidents/{incident.id}/')
        self.assertEqual(detail_response.status_code, 200, detail_response.data)
        self.assertEqual(detail_response.data['rca_report']['incident_id'], incident.id)
        self.assertEqual(detail_response.data['rca_report']['root_cause']['type'], AIOpsIncidentHypothesis.TYPE_ALERT_SYMPTOM)
    ...
    def test_readonly_investigation_partial_result_does_not_reuse_old_evidence(self):
        ...
        self.assertTrue(task.orchestration_state['rca_report']['partial'])
        self.assertEqual(task.orchestration_state['rca_report']['evidence']['observed'], [])
        self.assertEqual(task.result_payload['rca_report']['evidence_count'], 0)
        ...
        self.assertTrue(incident.metadata['last_rca_report']['partial'])
        self.assertEqual(incident.metadata['last_rca_report']['evidence_count'], 0)
