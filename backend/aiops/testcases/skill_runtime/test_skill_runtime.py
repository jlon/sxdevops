...
class BuiltinSkillRuntimeE2ETests(TestCase):
    ...
    def test_dispatch_runtime_stops_repeated_same_tool_arguments(self):
        ...
        self.assertEqual(result['metadata']['runtime_stop_reason'], 'tool_budget_stopped')
        self.assertTrue(result['metadata']['partial_answer'])
        self.assertEqual(result['metadata']['formatter_mode'], 'fallback')
